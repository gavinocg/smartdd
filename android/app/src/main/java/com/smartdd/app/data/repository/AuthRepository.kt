...
class AuthRepository @Inject constructor(
    ...
    suspend fun register(name: String, email: String, password: String): Result<RegisterResponse> = try {
        DebugLog.d("AuthRepo", "Register attempt: $email")
        ...
        if (response.isSuccessful && response.body() != null) {
            ...
            DebugLog.i("AuthRepo", "Register success: $email")
            Result.Success(body)
        } else {
            val msg = "Error de registro (${response.code()})"
            DebugLog.e("AuthRepo", msg, null)
            Result.Error(msg, response.code())
        }
    } catch (e: Exception) {
        DebugLog.e("AuthRepo", "Register exception: ${e.message}", e)
        Result.Error(e.message ?: "Error de conexión")
    }

    suspend fun login(email: String, password: String): Result<LoginResponse> = try {
        DebugLog.d("AuthRepo", "Login attempt: $email")
        val response = api.login(LoginRequest(email, password))
        if (response.isSuccessful && response.body() != null) {
            val body = response.body()!!
            tokenManager.saveTokens(body.token, body.refreshToken)
            tokenManager.saveUserInfo(body.user.id, body.user.name, body.user.email, body.user.plan, body.user.role)
            DebugLog.i("AuthRepo", "Login success: $email")
            Result.Success(body)
        } else {
        ...
        DebugLog.e("AuthRepo", "Login exception: ${e.message}", e)
        Result.Error(e.message ?: "Error de conexión")
    }
    ...
    suspend fun refreshToken(): Result<String> = try {
        ...
        if (response.isSuccessful && response.body() != null) {
            ...
            DebugLog.d("AuthRepo", "Token refreshed")
